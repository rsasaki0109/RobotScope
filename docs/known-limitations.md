...
Concrete scope boundaries for **v1.3.0-beta.0**. Product choices or not-yet-built — not necessarily bugs.
...
| **Live action gateway** | Opt-in **Allow action goals** + agent **`--allow-action`** allowlist — see [action-gateway.md](RobotScope/docs/action-gateway.md). **Fibonacci goal send** (v1.2) + **command bar sequence tracking** (v1.3 beta); no cancel/preempt or dedicated action panel |
