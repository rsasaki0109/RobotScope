# Known limitations (v1.2 GA)

Concrete scope boundaries for **v1.2.0**. Product choices or not-yet-built — not necessarily bugs.
...
| **Live action gateway** | Opt-in **Allow action goals** + agent **`--allow-action`** allowlist — see [action-gateway.md](RobotScope/docs/action-gateway.md). **Fibonacci goal send only** (v1.2); no feedback/result UI |
