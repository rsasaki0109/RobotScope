...
Concrete scope boundaries for **v1.0.0**. Product choices or not-yet-built — not necessarily bugs.
...
| **Live command gateway** | Opt-in **Allow publish** + agent **`--allow-publish`** allowlist — see [command-gateway.md](RobotScope/docs/command-gateway.md). **6-DOF Twist editor** (v1.0); no service/action gateway |
