# Known limitations (v1.4 beta)

Concrete scope boundaries for **v1.4.0-beta.0**. Product choices or not-yet-built — not necessarily bugs.
...
| **Live action gateway** | Opt-in **Allow action goals** + agent **`--allow-action`** allowlist — see [action-gateway.md](RobotScope/docs/action-gateway.md). **Fibonacci goal send** (v1.2) + **sequence tracking** (v1.3) + **Cancel goal** while running (v1.4 beta); no preempt UI or dedicated action panel |
