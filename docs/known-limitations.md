# Known limitations (v1.1 GA)

Concrete scope boundaries for **v1.1.0**. Product choices or not-yet-built — not necessarily bugs.
...
| **Live service gateway** | Opt-in **Allow service calls** + agent **`--allow-service`** allowlist — see [service-gateway.md](RobotScope/docs/service-gateway.md). **Trigger shortcut only** (v1.1); no action gateway |
